...
%% --- CHANGE LPR-001: version bump to 0.5 (running header) ---
\lhead{\textit{v0.5 $\cdot$ \today}}
\cfoot{\thepage}
...
\begin{titlepage}
  ...
  %% --- CHANGE LPR-002: version bump to 0.5 (title page) ---
  {\large Version 0.5 $\cdot$ \today\par}
  \vfill
...
is infeasible.
%% --- CHANGE REV-001: replacement requirement is enforced by the loop constraint, not input validation (consistent with the resolved Open-Questions bullet) ---
This is not an input-level validation check---no input quantity determines
it. It is enforced automatically by the hard loop constraint on the variance
(Section~\ref{sec:loop}), which renders infeasible any schedule that never
replaces the component within $[H{+}1, 2H]$.
...
%% --- CHANGE REV-002: derived constants added to the notation table (kappa_C, kappa, b, c, V_max^RF, big-M symbols were previously defined only in-line) ---
\midrule
\multicolumn{2}{@{}l}{\emph{Derived constants (precomputed from inputs; defined where introduced):}} \\
$\Phi^{-1}(1-\varepsilon)$ & Standard-normal quantile (Gaussian/Wiener reliability) \\
$V_{\max} = \tau^2/(\Phi^{-1}(1-\varepsilon))^2$ & Variance big-M (Gaussian, Wiener) \\
$\kappa_C = \sqrt{(1-\varepsilon)/\varepsilon}$ & Cantelli coefficient (Rainflow) \\
$\kappa = \ln(1/\varepsilon)$ & Bernstein exponent constant (Rainflow) \\
$b_{i\ell}$, $c_{i\ell} = \kappa\, b_{i\ell}/3$ & Bernstein support constant and margin (Rainflow, Eq.~\eqref{eq:rf_bsupport}) \\
$V_{\max}^{\mathrm{RF}}$ & Variance big-M for rainflow components (Eq.~\eqref{eq:rf_vmax}) \\
$\bar{\alpha}_{i\ell} = \tau/\beta_{i\ell}$, $\hat{\alpha}_{i\ell}$ & Gamma shape big-M and precomputed reliability cap \\
$\bar{\mu}_{i\ell}$ & IG precomputed reliability cap (root of Eq.~\eqref{eq:ig_mubar}) \\
\bottomrule
\end{longtable}
...
%% --- CHANGE REV-003: tau may be scalar or (F, L), consistent with the shape-validation list, the base.py signature, and the HDF5 schema; a scalar is broadcast ---
$\tau$ & \texttt{float} or $F \times L$ & Degradation upper bound (must be positive
  element-wise; a scalar is broadcast to $(F, L)$; the unsubscripted $\tau$ in the
  constraint sections stands for the per-component value after broadcasting) \\
...
%% --- CHANGE LPR-003: C_P parameter removed (soft loop penalty dropped) ---
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Shape validation.} Before broadcasting, the solver shall validate:
        ...
        %% --- CHANGE REV-004: stale legacy parameter alpha_target removed (it duplicated tau under its pre-v0.3 name and is defined nowhere in the input tables) ---
        $\rho, \tau$ must be scalar or $(F, L)$;
...
%% --- CHANGE REV-005: z formula qualified as the ARD1 form; mu_last / alpha_last applicability corrected (mu_last is Wiener-ARA1 only; Gamma-ARA1 tracks alpha_last) ---
$z_{i\ell k}$ & $F \times L \times 2H$, non-negative
  & Degradation removed by repair in mean units:
    $z_{i\ell k} = \rho_{i\ell}\,\mu_{i\ell,k-1}\,x^m_{i\ell k}$ (ARD1 form;
    ARA1 subtracts the anchor, Section~\ref{sec:repair_proxy})
    (bilinear; linearised via McCormick envelope) \\
$\mu^{\mathrm{last}}_{i\ell k}$ & $F \times L \times 2H$, non-negative
  & Mean at last maintenance epoch (ARA1 Wiener components only) \\
$\alpha^{\mathrm{last}}_{i\ell k}$ & $F \times L \times 2H$, non-negative
  & Shape at last maintenance epoch (ARA1 Gamma components only) \\
\bottomrule
\end{longtable}
...
%% --- CHANGE LPR-004: soft loop penalty term C_P * L_loop removed from objective; prose reworded ---
\begin{align}
\min \quad
...
The objective comprises four cost terms: maintenance-day cost, repair cost,
replacement cost, and the damage regularisation term, where
$z_{i\ell k} = \rho_{i\ell}\,\mu_{i\ell,k-1}\,x^m_{i\ell k}$ is the degradation
removed by repair in mean units (see Section~\ref{sec:repair_proxy}) and $u$ is
the damage regularisation variable defined by \texttt{penalty\_type}
(see Section~\ref{sec:objective}). Sustainability across the two half-horizons
is enforced by the \emph{hard} loop constraints of Section~\ref{sec:loop}; it
does not appear as an objective term.
...
%% --- CHANGE REV-006: idle-as-all-zeros semantics made explicit; C_M term is a genuine decision cost, not a constant ---
\emph{Idle semantics.} Idleness is encoded as the all-zero assignment row:
$x_{ijk} = 0$ for every $j \in \{0, 1, \ldots, M\}$ means train $i$ performs no
mission and is \emph{not} on a maintenance day at step $k$ (idle days add no
degradation increment, since $\delta\mu$, $\delta v$, $\delta\alpha$ sum over
$j \geq 1$ only). A maintenance day must be selected explicitly by setting
$x_{i0k} = 1$; the compatibility constraints below require this before any
repair or replacement can occur. Consequently the objective term
$C_M \sum_{i,k} x_{i0k}$ prices a genuine scheduling decision: because the
one-activity constraint is an inequality, the number of maintenance days is a
free, costed choice between idle and maintenance (under an equality convention
$\sum_j x_{ijk} = 1$ it would instead be the constant $F - M$ per step,
rendering the term vacuous).
...
%% --- CHANGE LPR-005: rewritten as two-mechanism remark (C_P mechanism removed); slack-robustness argument added ---
...
when $x^m_{i\ell k}=0$) is kept tight at optimality through two complementary
cost mechanisms:
...
\emph{Remark (the loop guarantee is robust to slack).}
Where neither cost mechanism bites, the tracked states may carry slack; this
never endangers the perpetual-safety guarantee of the loop constraints. The
dynamics rows are $\geq$-relaxations, so the tracked pair
$(\mu_{i\ell k}, v_{i\ell k})$ is a valid componentwise \emph{upper} bound on
the true moments: writing $\varphi_k$ for the exact one-step moment map induced
by the schedule at step $k$ (add the mission increment; scale by
$(1-\rho_{i\ell})$, resp.\ $(1-\rho_{i\ell})^2$, on repair; reset to
$(\mu^{\mathrm{new}}_{i\ell}, v^{\mathrm{new}}_{i\ell})$ on replacement), each
$\varphi_k$ is componentwise non-decreasing, every feasible point satisfies
$(\mu_{i\ell k}, v_{i\ell k}) \geq
\varphi_k(\mu_{i\ell,k-1}, v_{i\ell,k-1})$ row by row, and induction from the
exact initial condition gives domination of the true moments at every step.
Chaining these two facts over the second half yields
$(\mu_{i\ell,2H}, v_{i\ell,2H}) \geq \Phi(\mu_{i\ell,H}, v_{i\ell,H})$, where
$\Phi$ is the exact (monotone) second-half period map; combined with the hard
loop constraint
$(\mu_{i\ell,2H}, v_{i\ell,2H}) \leq (\mu_{i\ell,H}, v_{i\ell,H})$
(Section~\ref{sec:loop}) this gives
$\Phi(\mu_{i\ell,H}, v_{i\ell,H}) \leq (\mu_{i\ell,H}, v_{i\ell,H})$ --- exactly
the induction premise of the repeatability argument spelled out in
Section~\ref{sec:dyn_rainflow}, started from the envelope
$(\mu_{i\ell,H}, v_{i\ell,H})$. Since the reliability certificate is imposed on
the tracked states at every step $k = 1,\ldots,2H$ and is monotone in
$(\mu, v)$, every step of every future repetition of the second half is
dominated by a certified tracked point and therefore stays certified, even if
the tracked states carry slack. Removing the former soft loop penalty thus
costs only its tie-break toward larger sustainability margins among equal-cost
schedules, never the guarantee. The same argument applies unchanged to the
other model recursions and their loop constraints (IG mean-only, Gamma
shape-only, Wiener and rainflow moment pairs), each of whose one-step maps and
reliability certificates are likewise monotone in the tracked state.
%% --- CHANGE REV-007: slack-robustness coverage extended to ARA1 components (full state including the anchor; see the anchor loop rows added by REV-008/REV-009) ---
For ARA1 components the tracked state additionally includes the maintenance
anchor ($\mu^{\mathrm{last}}$, resp.\ $\alpha^{\mathrm{last}}$): the per-day
maps remain componentwise monotone in the \emph{full} state including the
anchor, and the loop constraints bound the anchor as well
(Sections~\ref{sec:dyn_wiener} and~\ref{sec:dyn_gamma}), so the premise
$\Phi(s_H) \leq s_H$ again holds on the full state. The anchor update rows
are two-sided, hence exact at binary points, and the domination induction
goes through with the anchor pinned to the (dominating) tracked mean.
...
%% --- CHANGE LPR-006: retitled hard-only; IG soft-penalty formula deleted below ---
\subsubsection{Loop Constraint (IG, hard)}
...
%% --- CHANGE REV-008: duplicated sentence fragment removed ---
...
%% --- CHANGE LPR-007: three cost mechanisms -> two (C_P mechanism removed) ---
\emph{Remark (Wiener variance tightness).}
Unlike the mean, whose tightness follows from the two cost mechanisms
...
%% --- CHANGE LPR-008: retitled hard-only; Wiener soft-penalty formula deleted below ---
\subsubsection{Loop Constraint (Wiener, hard)}
...
%% --- CHANGE REV-009: ARA1 anchor loop row added (closes the anchor gap: the chaining premise must hold on the full state, which for ARA1 includes mu_last) ---
For \texttt{ARA1} Wiener components the state carried across the horizon
boundary includes the maintenance anchor $\mu^{\mathrm{last}}$, so the loop
premise must hold on the \emph{full} state. One additional hard row is
imposed:
\begin{equation}
  \mu^{\mathrm{last}}_{i\ell,2H} \leq \mu^{\mathrm{last}}_{i\ell,H}
  \qquad \forall\,(i,\ell)\text{ s.t. model} = \mathrm{Wiener}
  \text{ and \texttt{maintenance\_type} \texttt{=} \texttt{ARA1}}
  \label{eq:w_loop_anchor}
\end{equation}

\emph{Remark (why the anchor row is needed and suffices: full-state
monotonicity).}
The chaining argument of Section~\ref{sec:dyn_rainflow} requires the premise
$\Phi(s_H) \leq s_H$ componentwise on the full tracked state $s$, together
with componentwise monotonicity of every per-day map. For ARA1 Wiener the
state is $s = (\mu, \mu^{\mathrm{last}}, v)$, and, writing
$a = \mu^{\mathrm{last}}$, the per-day maps of this section are:
mission/idle days,
$(\mu, a, v) \mapsto (\mu + \delta\mu,\; a,\; v + \delta v)$ (adds
non-negative constants); repair days,
$(\mu, a, v) \mapsto \bigl((1-\rho_{i\ell})\mu + \rho_{i\ell}\,a,\; \mu,\;
v\bigr)$, whose coefficient matrix on $(\mu, a)$ is
$\bigl(\begin{smallmatrix} 1-\rho_{i\ell} & \rho_{i\ell} \\ 1 & 0
\end{smallmatrix}\bigr)$ with all entries non-negative for
$\rho_{i\ell} \in (0,1]$; replacement days,
$(\mu, a, v) \mapsto (\mu^{\mathrm{new}}_{i\ell},\; a,\;
v^{\mathrm{new}}_{i\ell})$ (constant in $\mu, v$; identity in $a$). Every
per-day map is therefore componentwise non-decreasing in the full state
including the anchor, hence so is the second-half period map $\Phi$ and each
of its prefixes. Together with Eq.~\eqref{eq:w_loop_anchor}, the three hard
loop rows state exactly $\Phi(s_H) \leq s_H$, and the induction of
Section~\ref{sec:dyn_rainflow} applies coordinatewise; the reliability
certificate depends only on the $(\mu, v)$ coordinates and is monotone in
them, so every step of every future repetition stays certified. Without
Eq.~\eqref{eq:w_loop_anchor} the premise can fail on the $a$ coordinate: an
anchor that has grown by $k = 2H$ weakens every subsequent repair (the
post-repair mean $(1-\rho_{i\ell})\mu + \rho_{i\ell}\,a$ is increasing in
$a$), and the half-to-half domination breaks down even when the $\mu$ and $v$
rows hold.
...
%% --- CHANGE LPR-009: retitled hard-only; Gamma soft-penalty formula deleted below ---
\subsubsection{Loop Constraint (Gamma, hard)}
...
%% --- CHANGE REV-010: ARA1 anchor loop row added (Gamma analogue of eq:w_loop_anchor) ---
For \texttt{ARA1} Gamma components the tracked state additionally includes
the anchor $\alpha^{\mathrm{last}}$, which must be bounded across the horizon
boundary as well:
\begin{equation}
  \alpha^{\mathrm{last}}_{i\ell,2H} \leq \alpha^{\mathrm{last}}_{i\ell,H}
  \qquad \forall\,(i,\ell)\text{ s.t. model} = \mathrm{Gamma}
  \text{ and \texttt{maintenance\_type} \texttt{=} \texttt{ARA1}}
  \label{eq:gamma_loop_anchor}
\end{equation}
The justification is identical to the ARA1 Wiener case
(Section~\ref{sec:dyn_wiener}, Eq.~\eqref{eq:w_loop_anchor}): the repair-day
map $(\alpha, \alpha^{\mathrm{last}}) \mapsto
\bigl((1-\rho_{i\ell})\alpha + \rho_{i\ell}\,\alpha^{\mathrm{last}},\;
\alpha\bigr)$ has non-negative coefficients, mission/idle days add
non-negative constants, and replacement is constant in $\alpha$ and the
identity on $\alpha^{\mathrm{last}}$, so every per-day map---hence the
second-half period map---is componentwise monotone in the full state
$(\alpha, \alpha^{\mathrm{last}})$, and the two hard rows give
$\Phi(s_H) \leq s_H$, the premise of the chaining argument of
Section~\ref{sec:dyn_rainflow}. FSD along future repetitions then follows
from the shape ordering at fixed $\beta$, exactly as for the ARD1 case.
...
%% --- CHANGE LPR-010: three cost mechanisms -> two (C_P mechanism removed) ---
Section~\ref{sec:dyn_gaussian} (two cost mechanisms for $\mu$; reliability
...
%% --- CHANGE LPR-011: retitled hard-only; Rainflow soft-penalty formula deleted ---
\subsubsection{Loop Constraint (Rainflow, hard)}
...
%% --- CHANGE REV-011: anchor index corrected k -> k-1 (the auxiliary update sets mu_last_k = mu_{k-1} on repair days, so the k-indexed difference would vanish identically; the k-1 index matches the ARA1 mean dynamics) ---
\begin{equation}
  z_{i\ell k} = \rho_{i\ell}\,(\mu^-_{i\ell,k} - \mu^{\mathrm{last}}_{i\ell,k-1})\,x^m_{i\ell k}
  \qquad \forall\, i,\ell,k \text{ (ARA1 components)}
\end{equation}
Note the $k{-}1$ index on the anchor: the amount removed is measured against
the anchor in force \emph{before} the day-$k$ update, matching the ARA1 mean
dynamics $\mu_{i\ell k} = \mu_{i\ell,k-1} - \rho_{i\ell}(\mu_{i\ell,k-1} -
\mu^{\mathrm{last}}_{i\ell,k-1})$. Using the day-$k$ anchor---which the
auxiliary update sets to $\mu_{i\ell,k-1}$ on repair days---would make the
product vanish identically.
...
% ----------------------------------------------------------------
%% --- CHANGE LPR-012: subsection restructured to hard constraints only; soft penalties, total L_loop sum, and MATH-016 remark deleted ---
\subsection{Loop Constraints (hard)}
\label{sec:loop}
% ----------------------------------------------------------------

The loop constraints enforce sustainability over the two half-horizons $[1,H]$
and $[H{+}1,2H]$. A \textbf{hard constraint} is applied for each model,
bounding the tracked state at $k = 2H$ by its value at $k = H$. The chaining
argument showing that these constraints certify every future repetition of the
second half is given in Section~\ref{sec:dyn_rainflow}; the slack-robustness
remark in Section~\ref{sec:dyn_gaussian} shows the guarantee survives slack in
the one-sided dynamics linearisation.
...
Hard constraint as defined in Section~\ref{sec:dyn_ig}.

\subsubsection{Wiener}
%% --- CHANGE RF-025: A-02 accepted review item applied (FSD wording corrected) ---
%% --- CHANGE REV-012: ARA1 anchor row mentioned (added by REV-009) ---
Hard constraints as defined in Section~\ref{sec:dyn_wiener}.
...
For \texttt{ARA1} components the maintenance anchor is bounded as well,
$\mu^{\mathrm{last}}_{i\ell,2H} \leq \mu^{\mathrm{last}}_{i\ell,H}$
(Eq.~\eqref{eq:w_loop_anchor}), so the loop premise holds on the full state.

\subsubsection{Gamma}
%% --- CHANGE REV-013: ARA1 anchor row mentioned (added by REV-010) ---
Hard constraint as defined in Section~\ref{sec:dyn_gamma}; for \texttt{ARA1}
components this includes the anchor row
$\alpha^{\mathrm{last}}_{i\ell,2H} \leq \alpha^{\mathrm{last}}_{i\ell,H}$
(Eq.~\eqref{eq:gamma_loop_anchor}).

%% --- CHANGE RF-026: rainflow loop cross-reference and total penalty term ---
\subsubsection{Rainflow}
Hard constraints as defined in Section~\ref{sec:dyn_rainflow}.
...
%% --- CHANGE LPR-013: C_P * L_loop term removed from (OBJ) ---
\begin{align}
\min\quad
  & C_M \sum_{k,i} x_{i0k}
...
\begin{alignat}{2}
  ...
  %% --- CHANGE REV-014: ARA1 anchor loop row in the full-problem block (REV-009) ---
  &\mu^{\mathrm{last}}_{i\ell,2H} \leq \mu^{\mathrm{last}}_{i\ell,H}
  &&\quad\text{(ARA1 only)} \tag{W: loop constraint (anchor)}
\end{alignat}
...
\begin{alignat}{2}
  ...
  %% --- CHANGE REV-015: ARA1 anchor loop row in the full-problem block (REV-010) ---
  &\alpha^{\mathrm{last}}_{i\ell,2H} \leq \alpha^{\mathrm{last}}_{i\ell,H}
  &&\quad\text{(ARA1 only)} \tag{$\Gamma$: loop constraint (anchor)}
\end{alignat}
...
%% --- CHANGE LPR-014: Loop penalty alignat block deleted (soft penalty removed; hard loop rows remain in the per-model blocks above) ---
\vspace{0.3em}
...
%% --- CHANGE REV-016: stale legacy parameter alpha_target removed from the signature (duplicate of tau under its pre-v0.3 name); replaced by epsilon, which the epsilon-range and Bernstein-precondition checks require ---
\begin{lstlisting}[language=Python]
def validate_inputs(
    ...
    tau: np.ndarray,          # (F, L)
    rho: np.ndarray,          # (F, L)
    epsilon: float,           # reliability threshold
...
%% --- CHANGE REV-017: stale alpha_target dataset removed (duplicate of tau under its pre-v0.3 name); epsilon persisted instead so the echoed parameters determine the reliability caps ---
\begin{lstlisting}
/metadata/
...
%% --- CHANGE LPR-015: C_P row removed from Always-Present fields ---
...
%% --- CHANGE REV-018: optionality reconciled with Section 3 (tail_bound, v_new, b_new have defaults and may be omitted; v_max_user is always optional) ---
Exceptions: \texttt{tail\_bound}, \texttt{v\_new}, and \texttt{b\_new} are
\emph{optional} with the stated defaults even when their model condition
holds (Sections~\ref{sec:input_scalars}ff.), and \texttt{v\_max\_user} is
always optional (no constraint is added when it is absent).
...
\texttt{tail\_bound} & \texttt{str} / $F {\times} L$ list & Rainflow (optional) &
...
%% --- CHANGE LPR-016: C_P line removed from YAML example ---
\begin{lstlisting}
F: 4
...
\begin{itemize}[leftmargin=1.5em]
  ...
  %% --- CHANGE LPR-017: loop-constraint bullet reworded hard-only (soft penalty removed in v0.5) ---
  \item[\checkmark] \textit{Resolved.} Loop constraints: hard for all models
        (the soft loop penalty and its coefficient $C_P$ were removed in v0.5;
        the hard constraints alone carry the perpetual-safety guarantee, see the
        slack-robustness remark in Section~\ref{sec:dyn_gaussian});
  ...
  \item[\textbf{FUTURE}] \textbf{Schedule-dependent Bernstein support.} The
        ...
        %% --- CHANGE REV-019: FUTURE bullet expanded per the Route-I refinements report (schedule-dependent support) ---
        A first-order version needing no running maximum is available: per-member
        service indicators $\zeta_{ij} \ge x_{ijk}$ (all $k$) activate, for each
        mission actually served, a conditional Bernstein row carrying that
        mission's own constant
        $\kappa \max\{\max_k b^{\mathrm{inc}}_{ij\ell k}, b^{0}_{i\ell},
        b^{\mathrm{new}}_{i\ell}\}/3$ (indicator form for the LP surrogate, big-$M$
        for the exact quadratic, whose body indicator constraints cannot carry),
        and because the certified value $\mu + c + \sqrt{c^2 + 2\kappa v}$ is
        increasing in $c$, this conjunction is exactly equivalent to
        Eq.~\eqref{eq:rf_bsupport} with the maximum restricted to the missions
        actually served---worked out, with a numerical example, in the project
        scoping document, subsection ``Two refinements of the Route-I
        constraints: schedule-dependent support and piecewise-linear
        tightening''.
  \item[\textbf{FUTURE}] \textbf{Multi-tangent PWL tightening of the rainflow LP
        ...
        %% --- CHANGE REV-020: FUTURE bullet expanded per the Route-I refinements report (multi-tangent PWL) ---
        The PWL equality realises the \emph{union} of the single-tangent regions
        (equivalently encodable as a disjunction with one selector binary per
        tangent point), never their conjunction, and its residual gap to the exact
        constraint is exactly $\min_r (\mu_{i\ell k} - \mu_r)^2$---quadratic in the
        distance to the nearest tangency point---as quantified with a worked
        example in the project scoping document, subsection ``Two refinements of
        the Route-I constraints: schedule-dependent support and piecewise-linear
        tightening''.
  \item[\textbf{FUTURE}] \textbf{ARA1 semantics for rainflow variance.} As for
        ...
        resolved.
  %% --- CHANGE REV-021: ARA1 anchor convention flagged (pre- vs post-repair storage; no reset at replacement); found while closing the anchor loop gap ---
  \item[\textbf{FUTURE}] \textbf{ARA1 anchor convention (pre- vs.\ post-repair)
        and reset at replacement.} The auxiliary recursions of
        Sections~\ref{sec:dyn_wiener} and~\ref{sec:dyn_gamma} store the
        \emph{pre-repair} state at each maintenance epoch
        ($\mu^{\mathrm{last}}_{i\ell k} = \mu_{i\ell,k-1}$ on repair days),
        whereas the ARA1 definition in the Project Overview (following
        Mercier \& Castro, 2019) takes $D^{\mathrm{last}}$ to be the
        \emph{post-repair} level. Under the stored convention, ``degradation
        accumulated since the last maintenance'' is measured from the higher
        pre-repair baseline, so a repair scheduled before the state has
        re-grown past the stored anchor has non-positive removal (the
        optimiser avoids such repairs, which increase both cost and state,
        but the semantics should be reconciled before implementation; storing
        the post-repair value $\mu^{\mathrm{last}}_{i\ell k} = \mu_{i\ell k}$
        on repair days keeps all rows linear and preserves the full-state
        monotonicity of Section~\ref{sec:dyn_wiener}). Relatedly, the anchor
        is \emph{not} reset at replacement ($x^r_{i\ell k}$ does not enter
        the auxiliary update), so after a replacement the next ARA1 repair
        measures accumulation against a stale anchor; resetting
        $\mu^{\mathrm{last}}_{i\ell k} = \mu^{\mathrm{new}}_{i\ell}$ (resp.\
        $\alpha^{\mathrm{last}}_{i\ell k} = \alpha^{\mathrm{new}}_{i\ell}$)
        when $x^r_{i\ell k} = 1$ is the natural fix and, being a constant
        map, also preserves the monotonicity argument. Neither issue affects
        the validity of the loop-constraint chaining---the recursions as
        written are componentwise monotone either way---but both affect model
        fidelity.
...
%% --- CHANGE REV-022: v0.5 changelog extended with the final review pass ---
