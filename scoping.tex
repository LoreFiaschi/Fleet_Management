...
Bernstein
  ...
  & Yes & No
  & exploits independence; few parameters; linear in the counts
...
\begin{equation}\label{eq:bernstein_constraint}
  ...
  \;\le\; \tau,
\end{equation}
a convex, second-order-cone-representable constraint in the assignment counts (equivalently $(\kappa b_\ell/3)^2 + 2\kappa\sum_j n_{ij} v_{\ell j} \le (\tau - \kappa b_\ell/3 - \sum_j n_{ij}\mu_{\ell j})^2$ with the bracket non-negative).
...
One further period of the dynamics maps these two distinct parameter points to \emph{different} bound values, so $U_{3H} > U_{2H} = U_H$ is possible: the certified bound can creep upward over repetitions, and the single-period inequality does not induct.
...
Both inequalities in~\eqref{eq:loop_moments} are linear in the assignment and maintenance variables, adding two linear constraints; they are strictly stronger than the single relation $U_{2H} \le U_H$, and that strengthening is exactly what makes the induction valid.
...
This chains \emph{provided the repeated block contains only additive increments and full replacement}: there $M_{D}(s_0)$ propagates multiplicatively, $M_{D + \Delta D}(s_0) = M_D(s_0)\,M_{\Delta D}(s_0)$, so it is a self-propagating scalar and~\eqref{eq:loop_cumulant} induct.
