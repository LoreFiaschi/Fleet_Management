        ...
        for l in range(L):
            for k in range(T):
                ...
                rows.append({
                    ...
                    "horizon": "first" if k < T // 2 else "repeated",
                })
...
def structural_checks(res: dict, tol: float) -> list[dict]:
    ...
    add("mu_periodic_2H_le_H",
        max(0.0, float(np.max(mu[:, :, T - 1] - mu[:, :, H - 1]))))
    if res.get("v") is not None:
        v = res["v"]
        add("v_periodic_2H_le_H",
            max(0.0, float(np.max(v[:, :, T - 1] - v[:, :, H - 1]))))
    return checks
...
def _schedule_heatmap(res: dict, rel_margin: np.ndarray, tol: float):
    ...
    # horizon divider (between first and repeated horizon)
    ax.axvline(H + 0.5, color="black", linewidth=2.0)
    ...
    try:
        ...
        if method == "hoeffding":
            if support_param is not None and xi is not None:
                R = reconstruct_state(res["x"], support_param ** 2, xi, H, None)
            else:
            ...
            if cgf_param is not None and xi is not None and s_chernoff is not None:
                K = reconstruct_state(res["x"], cgf_param, xi, H, None)
            else:
...
def _render_tabs(state: dict):
    ...
    with tab_overview:
        ...
        c2 = st.columns(4)
        c2[0].metric("F x L x 2H cells", len(df))
    ...
    with tab_reliability:
        ...
        for l in range(L):
            ...
            ax.axvline(T // 2 - 0.5, color="k", lw=1.5)
    ...
    with tab_repeat:
        ...
        st.write("Repeatability loops the **moments** (slide 36): for every vehicle "
                 "and component the repeated horizon must end no worse than the first, "
                 "i.e. **mu(2H) <= mu(H)** and **v(2H) <= v(H)**. "
                 "A margin is (first - repeated); it must be >= 0.")
        F, L, T = res["mu"].shape
        H = res["H"]
        ...
        for i in range(F):
            for l in range(L):
                mu_H, mu_2H = float(mu[i, l, H - 1]), float(mu[i, l, T - 1])
                row = {
                ...
                if v is not None:
                    v_H, v_2H = float(v[i, l, H - 1]), float(v[i, l, T - 1])
        ...
        fig, axes = plt.subplots(1, ncols, figsize=(max(len(rep_df) * 0.6, 6), 4),
                                 squeeze=False)
        axm = axes[0, 0]
        axm.bar(xpos - 0.2, rep_df["mu(H)"], width=0.4, label="mu(H)")
        axm.bar(xpos + 0.2, rep_df["mu(2H)"], width=0.4, label="mu(2H)")
        axm.set_xticks(xpos); axm.set_xticklabels(labels, rotation=90, fontsize=7)
        axm.set_title("mean: mu(H) vs mu(2H)"); axm.legend(); axm.grid(alpha=0.3)
        ...
        if v is not None:
            axv = axes[0, 1]
            axv.bar(xpos - 0.2, rep_df["v(H)"], width=0.4, label="v(H)")
            axv.bar(xpos + 0.2, rep_df["v(2H)"], width=0.4, label="v(2H)")
            axv.set_xticks(xpos); axv.set_xticklabels(labels, rotation=90, fontsize=7)
            axv.set_title("variance: v(H) vs v(2H)"); axv.legend(); axv.grid(alpha=0.3)
        fig.tight_layout()
        st.pyplot(fig)
        st.caption("A pair is repeatable when the repeated-horizon bar (2H) does not "
                   "exceed the first-horizon bar (H) for both mean and variance.")
    ...
    with tab_vehicle:
        ...
        ax.axvline(T // 2 - 0.5, color="gray", linestyle=":", label="H | repeated")
        ...
        if v is not None:
            ...
            axv.axvline(T // 2 - 0.5, color="gray", linestyle=":", label="H | repeated")
            for k in range(T):
                if act_i[k] == 0:
